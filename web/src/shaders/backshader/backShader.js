...
float shape(vec2 st, float radius) {
	st = vec2(0.5)-st;
    float r = length(st -vec2((u_resolution.x/ 2.)/u_resolution.y,0.6));
...
void main() {
    ...
    float r = length(st -vec2((u_resolution.x/ 2.)/u_resolution.y,0.5));
    float a = atan(st.y, st.x);
    ...
    color += mix(color.r, color.g, r*-0.438)*noise(st);
    color += mix(color.r, color.b, r*1.522*noise(st)*sin(u_time)*10.984);
    
	color += vec3(1.0) *noise(st);
